...
function mostrarDatosEnPerfil(albumes) {
  ...
  // 1.cantidad de albumes en el array
  const spanCantAlbums = document.getElementById("cant-albums");
  const countAlbumes = albumes.length;
  let contentSpanCant = `Hay ${countAlbumes}`;
  if (countAlbumes > 1 || countAlbumes === 0) {
    contentSpanCant = contentSpanCant + " álbumes";
  } else if (countAlbumes === 1) {
    contentSpanCant = contentSpanCant + " álbum";
  }
  ...
  if (countFavorites > 1 || countFavorites === 0) {
    contentSpanFavorites = contentSpanFavorites + " favoritos";
  } else if (countFavorites === 1) {
    contentSpanFavorites = contentSpanFavorites + " favorito";
  }
