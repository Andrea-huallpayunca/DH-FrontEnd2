  ...
  botonesDeLike.forEach((boton) => {
    boton.addEventListener("click", function (evento) {
      ...
      albumesFamosos.forEach((album) => {
        if (album.id == albumId) {
          console.log(
            `Coinciden los álbumes ${albumId} es igual al ${album.id}`
          );
...
window.addEventListener("keydown", function (event) {
  // keydown todas las teclas del teclado
  //   console.log(event);
  //   console.log(event.key);
  //   console.log(event.code);
  const letraPresionada = event.code;
  if (letraPresionada === "KeyF") {
    // Paso 2
    const albúmParaEliminar = prompt("¿Qué albúm desea eliminar?");
    
    // Paso 3
    const posiciónAlbum = albumesFamosos.findIndex(
      (element) => element.nombre === albúmParaEliminar
    );
    
    // Paso 4
    if (posiciónAlbum != -1) {
      albumesFamosos.splice(posiciónAlbum, 1);
      // Paso 5
      renderizarAlbumes(albumesFamosos);
      marcarFavorito();
    } else {
      alert("El nombre del álbum ingresado no existe.");
    }
  }
});
